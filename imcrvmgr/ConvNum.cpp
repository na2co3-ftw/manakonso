...
std::wstring ConvNum(const std::wstring &key, const std::wstring &candidate)
{
	...
	LPCWSTR numtype_10k[] = {L"", L"万", L"億", L"兆", L"京", L"垓",
		L"𥝱", L"穣", L"溝", L"澗", L"正", L"載", L"極", L"恒河沙", L"阿僧祇", L"那由他", L"不可思議", L"無量大数"};
	LPCWSTR numtype6[][10] = {
	...
	while(std::regex_search(candidate_tmp, result, rxint) && (j < keynum.size()))
	{
		...
		case L'5':	//大字
			...
			break;
		case L'6':	//ローマ数字(独自拡張)
			if(keynum[j].size() > 4 || (keynum[j].size() == 4 && keynum[j][0] >= L'4'))
			{
				repcandidate.append(keynum[j]);
			}
			else
			{
				...
				for(i = 0; i < k; i++)
				{
					repcandidate.append(numtype6[k - i - 1][keynum[j][i] - L'0'/*0x0030*/]);
				}
			}
			break;
		case L'7':	//未定義
